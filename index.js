...
app.set('trust proxy', true); // Necessary to get the correct IP when behind a proxy like Railway
...
app.post("/log", (req, res) => {
  ...
  if (message === undefined) {
    console.log(`[${ip}] Received request with no message in body`);
    return res.status(400).json({ success: false, message: "'message' field is missing in the request body" });
  }
  
  console.log(`[${ip}] Received message:`, message);
  
  const newLogEntry = { 
    type: 'new_log', 
    ip: ip, // Include IP in the log entry
    content: message, 
